...
The different algorithms available are accessible from the {\tt coniii.solvers} module: {\tt Enumerate}, {\tt MPF}, {\tt Pseudo}, {\tt ClusterExpansion}, {\tt MCH}, and {\tt RegularizedMeanField} algorithms as described in the above sections. These algorithm classes are all derived from a base {\tt Solver} class. In the accompanying release version of the package, v1.0.0, each class has a {\tt solve} method for solving the inverse maxent problem. Furthermore, the parent {\tt Solver} class has a data member named {\tt Solver.sampler} that is a instance of a sampling class for obtaining a Monte Carlo realization from the maxent model. For the Ising model, this is typically an instance of {\tt FastMCIsing} from the {\tt coniii.solvers} module.
