...
\caption{Minimum probability flow involves minimizing the flow of probability between the data distribution $p^{(0)}(s)$ to a nearby distribution $p^{(\epsilon)}(s)$ obtained from running deterministic dynamics for  small time $\epsilon$ as represented by the black line. When the dynamics are run for a long time, we obtain the stationary distribution $p^{\infty}(s)$.}
\label{default}
\end{figure}
