...
Model selection involves a tradeoff between a detailed, specific model that can explain the data well with the ability to generalize across an ensemble of data sets resulting from differing noise or sampling conditions. In models of biological or social phenomena, it may be important to also consider the computational costs or physical limitations that restrict the class of models. These tradeoffs are concisely contained in the Bayesian posterior probability
\begin{align}
	p({\rm model}|{\rm data}) &= \frac{p({\rm data}|{\rm model})p({\rm model})}{p({\rm data})}\\
	{\rm posterior} &= \frac{{\rm likelihood}\times{\rm prior}}{{\rm evidence}}
\end{align}
In other words, the most likely model is the one that is supported by the data relative to the prior (and we can ignore the denominator as a normalizing factor unless we are considering multiple data sets). The prior can reflect either the cost of describing a particular model (it's really detailed and takes costs a lot of information or energy and thus unlikely) or it can reflect the initial set of assumptions on the part of the scientist about what models are important to consider.

Maximum entropy provides a natural framework for model selection because we can construct a hierarchy of more complicated models by adding more constraints. Since including additional constraints must always reduce or maintain the entropy of the model, we can order a sequence of models as a monotonically decreasing the entropy til the probability distribution has been matched exactly. This entropic difference is
\begin{align}
	\Delta S_{\rm model} &= S[p_{\rm model}] - S[p_{\rm data}] \geq 0
\end{align}

With the Ising model, such a sequence can be constructed by constraining kth order moments in order $\br{\sigma_{\rm i_1}\sigma_{\rm i_2}\cdots\sigma_{\rm i_k}}$ where the entropy decays monotonically between the 1st order model to the $k=N$th order model. Normalizing this distance to one, we construct the multi-information:
\begin{align}
	0\leq \Delta S_{\rm n}/\Delta S_{\rm 1} \leq 1
\end{align}
...
In principle, we could construct our models with whatever set of constraints we consider to be important [isn't there a schneidman paper about this?], but how do we choose when to stop? Clearly, an exact match to the data distribution with as many parameters as is sufficient to describe it exactly is not very useful. In other words, there must be some tradeoff between descriptive power of the model and the cost of describing it. There are a number of ways to consider how models compare with one another like the Akaike Information Criterion (AIC).
